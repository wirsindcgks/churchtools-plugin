...
- **Schutz gegen eine leere API-Antwort.** `Client::request()` liefert auch bei HTTP 200 mit unerwartetem Body ein leeres Array zurück, ohne zu werfen – geändertes Response-Format, ein Kalender verliert still die Leseberechtigung, eine Fehlerseite von einem Proxy. `deleteOrphans()` lässt bei leerer Keep-Liste seine `NOT IN`-Schutzbedingung komplett weg und hätte dann jeden künftigen Termin aller aktiven Kalender gelöscht, gefolgt von der Meldung „Synchronisation abgeschlossen". Der Schaden wäre nicht dauerhaft gewesen – der Sync ist ein vollständiger Spiegel, der nächste erfolgreiche Lauf legt alles wieder an –, bis dahin hätte die Gemeindeseite aber „Keine anstehenden Termine" gezeigt. Kommt jetzt nichts zurück, obwohl gespeicherte Termine vorliegen, bricht der Lauf ab, löscht nichts und meldet den Fehler. Bewusst nur die Grenze „null gegen nicht-null" statt einer prozentualen Schwelle, an der ein wirklich schrumpfender Kalender dauerhaft hängenbliebe
- **Hinweis im Backend, wenn der Sync klemmt.** Ein fehlgeschlagener oder stehengebliebener Sync stand bisher nur im Tab „Übersicht" – wer nicht gezielt hinsieht, merkt wochenlang nicht, dass die Termine eingefroren sind, weil eine veraltete Terminliste völlig normal aussieht. Neue `Admin\SyncHealthNotice` meldet auf jeder Admin-Seite: letzter Lauf fehlgeschlagen, kein Zeitplan hinterlegt, oder letzter erfolgreicher Lauf älter als das Dreifache des eingestellten Intervalls (WP-Cron ist von Haus aus unpünktlich, ein knapperer Faktor wäre Rauschen). Erscheint nicht auf einer frisch installierten, noch nicht eingerichteten Instanz und nicht auf der Plugin-Seite selbst
